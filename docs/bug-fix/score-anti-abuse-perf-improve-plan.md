...
- malicious_rule_type：AUTO / THRESHOLD，默认 AUTO。
- malicious_score_lower：阈值下限，可空。
- malicious_score_upper：阈值上限，可空。

含义如下：

- AUTO：继续沿用当前 MAD 低分检测。
- THRESHOLD：管理员配置总分区间，超出范围判定为恶意。

### 4.2 打分记录字段

建议在 scoring_record 上补充可审计字段：

- malicious_source：AUTO_MAD / THRESHOLD / MANUAL。
- malicious_reason：文本说明。

如果短期不想扩表，也可以先只保留现有 is_malicious，后续再补审计字段。

### 4.3 统计汇总表

如果要长期稳定落地，建议新增统计汇总表，至少拆成三类：

- project_statistics_summary：项目级总体统计。
- project_statistics_group_summary：小组统计。
- project_statistics_indicator_summary：指标统计。

可以再按需要补一个 scorer_distribution_summary 维护评委分布。

建议字段不要存原始明细，只存聚合结果，例如：

- raw_average_score
- processed_average_score
- abnormal_count
- sample_size
- valid_sample_size
- updated_time
